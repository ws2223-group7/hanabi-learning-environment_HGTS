...
class RewardShapeConverter:
    ...
    def convert(self, reward_shape: RewardShape) -> RewardShapeResult:
        ...
        successfully_played_a_card: float = 0.1 if reward_shape.successfully_played_a_card is True else -10

        discard: float = reward_shape.discard
        discard_playable: float = 0 if reward_shape.discard_playable is None else -10 if reward_shape.discard_playable is True else 0
        discard_unique: float = 0 if reward_shape.discard_unique is None else -10 if reward_shape.discard_unique is True else 0
        discard_useless: float = 0 if reward_shape.discard_useless is None else 10 if reward_shape.discard_useless is True else 0

        hint: float = 0.1 if reward_shape.hint is True else 0
        play: float = 0.1 if reward_shape.play is True else 0
